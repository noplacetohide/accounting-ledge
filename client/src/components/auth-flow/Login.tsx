...
export default function Login({ setIsLogin }: AuthFlowProps) {
    ...
    async function onSubmit(formState: z.infer<typeof formSchema>) {
        try {
            ...
            console.log(error)
            const errorMsg = error?.response?.data?.message || 'Login attempt failed!';
        ...
        <div className='h-screen w-screen bg-gray-200 flex justify-center items-center'>
            <Card className="w-[350px]">
                ...
                <CardContent>
                    ...
                    <Button type='button' onClick={() => setIsLogin(false)} className={`w-full cursor-pointer`}> <UserPlus /> Open Account</Button>
                </CardContent>
            </Card>
        </div>
    )
